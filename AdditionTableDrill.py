...
class AdditionTableDrill(AbstractDrill):
    drill_param_list = ["rand_ord", "max_num", "inc_zero", "inc_one", "inc_ten", "tri"]
    drill_param_text = {"rand_ord": "Randomize number order in row and column",
                        "max_num": "Maximum number in drill (minimum 9, maximum 17, default 10)",
                        "inc_zero": "Include 0?", "inc_one":"Include 1?", "inc_ten": "Include 10?",
                        "tri": "Triangular layout"}
    #0: binary default no, 1: binary default yes, 2: text entry
    drill_param_input = {"rand_ord": 0, "max_num": 2, "inc_zero": 0, "inc_one": 1, "inc_ten": 1, "tri": 1}
    ...
    def build_drill_pdf(cls, params):
        ...
        max_num = 10 if max_num == '' else min(max(int(max_num), 9), 17)
